          ...
          filteredRequests.map((request, index) => (
            ...
            >
              ...
              <div className="mt-5 space-y-3">
                <MobileInfoRow icon={Phone} label="Phone Number" value={request.mobileNumber} />
                <MobileInfoRow icon={Droplets} label="Units" value={String(request.units)} />
                <MobileInfoRow icon={MapPin} label="City" value={request.city} />
                <MobileInfoRow icon={CalendarDays} label="Requested On" value={formatDate(request.createdAt)} />
              </div>
...
function BloodRequestDetailModal({
  ...
  const [recordingDonorId, setRecordingDonorId] = useState<number | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    async function loadDetail() {
      try {
        ...
        if (!cancelled) {
          setError(getApiErrorMessage(err, "Unable to load this blood request."));
        }
      } finally {
        if (!cancelled) {
          setLoading(false);
          ...
          {!loading && !error && detail && (
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                ...
                <InfoTile label="Date of Birth" value={formatDate(detail.dateOfBirth)} />
              </div>
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                ...
                <InfoTile
                  label="Address"
                  value={[detail.address, detail.city, detail.district, detail.pincode]
                    .filter(Boolean)
                    .join(", ") || "—"}
                />
              </div>
              ...
              {/* MATCHED CENTRES */}
              <Section title={`Matched Centres (${detail.matchedCentres.length})`}>
              ...
              {/* DONOR CANDIDATES */}
              <Section title={`Donor Candidates (${detail.donorCandidates.length})`}>
                    ...
                    {detail.donorCandidates.map((donor) => (
                      ...
                      >
                        <div className="min-w-0">
                          ...
                          <p className="text-[11px] text-[var(--color-text-placeholder-alt)]">
                            {donor.mobileNumber} · {donor.bloodGroup} · {donor.city}
                          </p>
                        </div>
...
function InfoTile({ label, value }: { label: string; value: string }) {
  return (
    <div className="min-w-0 rounded-lg border border-[var(--color-border-lighter)] p-3">
      <p className="text-[11px] text-[var(--color-text-placeholder-alt)]">{label}</p>
...
function StatusBadge({ status }: { status: BloodRequestStatus }) {
  const styles: Record<BloodRequestStatus, string> = {
    CENTRES_FOUND: "border-[#c9e8d1] bg-[var(--color-success-bg)] text-[var(--color-success)]",
    NO_CENTRES_FOUND: "border-[#f5e2b8] bg-[#fff8e8] text-[#b8860b]",
    CLOSED: "border-[var(--color-border-lighter)] bg-[var(--color-surface-alt)] text-[var(--color-text-secondary)]",
