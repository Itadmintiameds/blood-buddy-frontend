...
export function RecipientSearchScreen() {
  ...
  return (
    <ScreenShell>
      <BrandHeader
        title="Search Blood Banks & Donors"
        showBackButton
        ...
        <section className="mx-auto w-full max-w-[720px] px-4 pb-10 pt-5 sm:px-6 sm:pt-7">
          ...
          <div className="mt-4 flex h-[160px] w-full items-center justify-center rounded-2xl border border-dashed border-[var(--color-border)] bg-white">
            <div className="text-center">
              ...
              />
              <p className="mt-1.5 text-[12px] text-[var(--color-text-placeholder-alt)]">
          ...
          <div className="mt-5 flex gap-2">
            <button
              ...
              className={`flex h-9 items-center gap-1.5 rounded-full px-4 text-[13px] font-semibold transition-all duration-200 ${
                tab === "bloodBanks"
          ...
          <div className="mt-4 space-y-3">
            {loading && (
              <div className="rounded-2xl border border-[var(--color-border-lighter)] bg-white px-4 py-6 text-center text-[13px] text-[var(--color-text-muted)]">
                Searching...
              </div>
            )}
            ...
            {!loading && tab === "bloodBanks" &&
              (bloodBanks.length === 0 ? (
                <EmptyState message="No blood banks found nearby." />
              ) : (
                bloodBanks.map((bank) => (
                  <div
                    key={bank.id}
            ...
            {!loading && tab === "donors" &&
              (donors.length === 0 ? (
                <EmptyState message="No donors found nearby." />
              ) : (
                donors.map((donor) => (
                  <div
                    key={donor.id}
